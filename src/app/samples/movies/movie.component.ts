...
@Component({
  ...
  template: `
    ...
    <div>
      <button mat-flat-button color="primary" (click)="previousMovie()">
        Previous
      </button>
      ...
      <button mat-flat-button color="primary" (click)="nextMovie()">
        Next
      </button>
    </div>
  `,
    ...
    this.currentMovie$ = this.route.paramMap.pipe(
      ...
      switchMap((id) => {
        return this.filmService
          .getFilm(id)
          .pipe(tap((movie) => (this.currentId = movie ? movie.id : 0)));
      })
    );
  }
